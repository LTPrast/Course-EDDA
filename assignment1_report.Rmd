...
```
Plots show a very not normal data distribution which is confirmed with Shapiro's test.
Equal variances:
```{r, fig.height=3, echo=-1}
par(mfrow=c(1,2))
...
```
F test significant so significant differences between variances so can't use assumption of equal variances in t-test.

The seeded and unseeded clouds data set is not paired data. In the experiment design it is described as out of a sample of 52 clouds 26 are chosen, this already implies the clouds are separate 'individuals'. Not only that, clouds are unique and when it rains the cloud disappears (at least changes), which makes it impossible to use the same cloud twice. So we use an unpaired two samples t-test:
```{r, output.lines=5:6}
...
Mann-Whitney and Kolmogorov need the samples to be independent. Due to the unique nature of clouds and since the clouds are chosen from the same sample this assumption is withheld. 
T-test isn't applicable due to assumption of normality being violated.

Non parametric tests are more significant due to outliers for which these tests aren't as sensitive for. Mann-Whitney is a rank test (median) and Kolmogorov checks the differences of the verticality of the histograms, since outliers are infrequent, verticality of histogram for the outlier small.
...
```
An improvement can be seen in the plots and Shapiro test. However the data still does not have a normal distribution.
...
```
There is still a significant difference in variance between the groups
...
While the data set still contains outliers they are less large. T-test assumptions are still violated even though the resulting p-value is more significant. The transformation doesn't affect ranked tests nor Kolmogorov Smirnoff test since the rank and frequency of the data points is still the same.
...
```
The data set is now normally distributed.
...
P-values are similar, all around 0,01.The t-test is slightly more significant, which is to be expected due the power of the t-test. With out the remaining outlier it would most likely be even more significant.
...
CI around lambda: [0.00139,lambda_est,0.00313]
...
We can see from the QQ plot that the residuals are not normally distributed, and the spread in the residuals shows that there is no equal variance in the residuals, so both the normality and the equal variances assumptions of the one-way ANOVA test are not met.
...
```
p-value > 0.05 so factor variety is not significant. So no, the two tests don't arrive at the same conclusion. While the one-way ANOVA test has more power than the Kruskal-Wallis test, it might be less accurate in this situation because both the normality and the equal variances assumptions of the one-way ANOVA test are not met, while the Kruskal-Wallis test does not rely on the normality.
...
anova(hemoglobinaov)
```
The p-value for the interaction between method and the rate is larger than 0.05. So, there is no evidence for interaction between the two factor. Therefore we will create a new model without the interaction called the additive model.
...
```
P-value > 0.05, therefore passed the test and can state that there exists no significant departure from normality. 
```{r, output.lines=6:7, fig.height=3}
plot(fitted(hemoglobinaov2),residuals(hemoglobinaov2))
hemoglobinaov2
```
The spread in the residuals seems to be in the similar range for all the fitted values. So, we can assume equal variance in the residuals. Because the requirements of normality and the assumption of equal variances are met, we can use the two-way ANOVA test.
...
Rate 2 and method B leads to a mean of 10.1, which is the highest hemoglobin yield when both factors are considered. When rate 3 and method A was used then the mean was eastimated to be 9.03.
...
QQplot is straight enough so assumption of normality is not violated and looking at the residuals plot the assumption of homogeneity of variances is also not violated. So the chosen Anova test is applicable.
...
summary(aovcream)
```
Batch is not an insignificant block variable, even though in the experiment design the batch was meant to be identical. However the position has no influence so can be considered an insignificant block element and be removed. The now two-way repeated measures Anova:
...
summary(aovcream)
```
Using the same model as in b, however now the block model batch is model with a random effect. Since lmer doesn't give p-values, it is compared to the same model with out starters. The variance between these models gives us the significance of the starter. Comparing the random effects model here with the fixed effect model in b), Starter 4 has the most significant effect on acidity in both models in lmer (T = 6.660). So similar results are found.
