...
```
Plots show a very not normal data distribution which is confirmed with Shapiro's test. Box plots show the data consists of many outliers. 
...
boxplot(sqrtseeded);boxplot(sqrtunseeded)
```
...
boxplot(sqrtsqrtseeded);boxplot(sqrtsqrtunseeded)
```
Normality test:
```{r, output.lines=-(1:4)}
...
```
Testing whether the data follows an exp(lambda_est) distribution: the H0 (that the data distribution is the same) cannot be rejected (both bootstrap test and Kolmogorov are insignificant). Thus we conclude that the distribution of the data follows a exp(lambda_est) distribution.
***d)***
