...
```
An improvement can be seen in the plots and 
...
Mann-Whithey and Kolmogorov were applicable the whole time, however now since only 1 outlier is left, t-test assumptions are not violated. 
...
```
Not significant (P-value = 0.9622) so h0 not rejected: which is medians are the same
...
```
Not significant (P-value = 0.08019) h0 not rejected: which is medians are the same
...
Position no significant effect (P = 0.411). However batch does have significant effect (P =0.00163) (not expected in question) and starter aslo has a significant effect (P = 2.904e-05). 
From summary: p value starter2 no significant (p = 0.754), this is compared to starter1.
QQplot is quite straight so assumption of normality is not violated and looking at the residuals plot the assumption of homogeneity of variances is also not violated. So the chosen Anova test is applicable.
...
```
Friedman test is a non parametric version of the repeated measures Anova. Whilst since the the assumptions for the repeated measure Anova are met, it makes more sense to use that test, no assumptions are violated **(double check this, difficult to check for effect starter, need to use post hoc test)** for the Friedman test, so it is possible to use it with acidity as response values, starter as the group and batch as block variables.
...
summary(aovcream)
```
Using the same model as in b, however now the block model batch is model with a random effect. Since lmer doesn't give p-values, it is compared to the same model with out starters. The variance between these models gives us the significance of the starter **(need to do this?, what did he say about random effect model in lecture)**, Comparing the random effects model here with the fixed effect model in b), Starter 4 has the most significant effect on acidity in both models in lmer (T = 6.660). So similar results are found.
